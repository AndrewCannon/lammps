  ...
  for (ii = 0; ii < inum; ii++) {
	...
	if ((itype==1) && (jtype==1)) { // fluid-fluid interaction outside of flux
	  ...
	  if (r <= phase_support[itype][jtype]) {
	    ...
	    deltaDcC = 1.0*RC[j]/faraday*(cC[i]*exp(transCoefC*faraday*overPot/R/T) - mM[j]*exp(-1*transCoefM*faraday*overPot/R/T));
	    dcC[i] -= deltaDcC;
	  }
	  ...
	  if (r <= phase_support[itype][jtype]) {
	    ...
	    // Butler - Volmer equations for reaction at solid fluid interface
	    deltaDmM = (jmass)*RC[i]/faraday*(cC[j]*exp(transCoefC*faraday*overPot/R/T) - mM[i]*exp(-1*transCoefM*faraday*overPot/R/T));
	    dmM[i] += deltaDmM;
